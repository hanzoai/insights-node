...
const createClient = (options) => {
    ...
    const client = new Insights('key', options)
...
test.before.cb((t) => {
    express()
        ...
        .post('/batch', (req, res) => {
            ...
            if (ua !== `@hanzo/insights-node/${version}`) {
                return res.status(400).json({
...
function callsDecide(expectedData) {
    const config = {
        ...
        headers: {
            ...
            'user-agent': `@hanzo/insights-node/${version}`,
        },
    }
    if (expectedData) {
...
test('expose a constructor', (t) => {
    t.is(typeof Insights, 'function')
})

test('require a api key', (t) => {
    t.throws(() => new Insights(), { message: "You must pass your Hanzo Insights project's api key." })
})
...
test('default options', (t) => {
    const client = new Insights('key')

    t.is(client.apiKey, 'key')
    t.is(client.host, 'https://insights.hanzo.ai')
    t.is(client.flushAt, 20)
    t.is(client.flushInterval, 10000)
})

test('remove trailing slashes from `host`', (t) => {
    const client = new Insights('key', { host: 'http://google.com///' })
...
test('identify - enqueue a message', (t) => {
    ...
    const apiMessage = {
        ...
        event: '$identify',
        properties: { $lib: '@hanzo/insights-node', $lib_version: version },
    }
...
test('capture - enqueue a message with groups', (t) => {
    ...
    const apiMessage = {
        distinctId: '1',
        properties: { $groups: { company: 'id: 5' }, $lib: '@hanzo/insights-node', $lib_version: version },
        event: 'event',
    }
...
test('alias - enqueue a message', (t) => {
    ...
    const apiMessage = {
        properties: { distinct_id: 'id', alias: 'id', $lib: '@hanzo/insights-node', $lib_version: version },
        event: '$create_alias',
        distinct_id: 'id',
    }
...
test('feature flags - ignores logging errors when insights:node is not set', async (t) => {
